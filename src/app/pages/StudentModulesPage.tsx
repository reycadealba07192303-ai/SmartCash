    ...
    useEffect(() => {
        ...
        const load = async () => {
            try {
                ...
                const modRes = await fetch('https://smartcash-x4j5.onrender.com/api/student/modules', {
                ...
                // Fetch Quiz History
                const histRes = await fetch('https://smartcash-x4j5.onrender.com/api/student/quizzes/history', {
                    headers: { 'Authorization': `Bearer ${token}` }
                });
                if (histRes.ok) {
                ...
                const enriched = await Promise.all(modsData.map(async (mod) => {
                    try {
                        const qRes = await fetch(`https://smartcash-x4j5.onrender.com/api/student/quizzes/module/${mod.id}`, {
                            headers: { 'Authorization': `Bearer ${token}` }
                        });
                        if (qRes.ok) {
