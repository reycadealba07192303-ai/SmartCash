...
const AdminUsersPage: React.FC = () => {
    ...
    const fetchUsers = async () => {
        try {
            const res = await fetch('https://smartcash-x4j5.onrender.com/api/admin/users', {
    ...
    const handleSaveUser = async () => {
        try {
            const res = await fetch(`https://smartcash-x4j5.onrender.com/api/admin/users/${selectedUser.id}/status`, {
                method: 'PATCH',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${token}`
    ...
    const confirmDeleteUser = async () => {
        try {
            const res = await fetch(`https://smartcash-x4j5.onrender.com/api/admin/users/${selectedUser.id}`, {
                method: 'DELETE',
                headers: { 'Authorization': `Bearer ${token}` }
            });
            if (res.ok) {
