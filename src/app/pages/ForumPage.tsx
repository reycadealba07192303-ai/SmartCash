...
const ForumPage: React.FC = () => {
    ...
    useEffect(() => {
        const fetchPosts = async () => {
            if (!token) return;
            try {
                const res = await fetch('https://smartcash-x4j5.onrender.com/api/student/forum/posts', {
    ...
    const handlePostSubmit = async (e: React.FormEvent) => {
        ...
        if (token) {
            try {
                await fetch('https://smartcash-x4j5.onrender.com/api/student/forum/post', {
                    method: 'POST',
                    headers: {
    ...
    const handleLike = async (postId: string) => {
        if (!token) return;
        try {
            const res = await fetch(`https://smartcash-x4j5.onrender.com/api/student/forum/posts/${postId}/like`, {
                method: 'PUT',
                headers: { 'Authorization': `Bearer ${token}` }
            });
            if (res.ok) {
    ...
    const handleToggleComments = async (postId: string) => {
        ...
        if (!comments[postId] && token) {
            try {
                const res = await fetch(`https://smartcash-x4j5.onrender.com/api/student/forum/posts/${postId}/comments`, {
                    headers: { 'Authorization': `Bearer ${token}` }
                });
                if (res.ok) {
                    const data = await res.json();
